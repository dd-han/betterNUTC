...
BRW_REF="http://academic.nutc.edu.tw/curriculum/next_show_subject/show_subject_form.asp"
...
function tab_header(){
	## 只是產生標題列而已XD
	echo "學制	修課班級	學年/學期	必選修	上課時間	修課科目	授課老師	學分數	時數	修課人數	教學大綱" > "$OUTPUT"
}

function get_lesson(){
	## 取得課程清單
	curl http://academic.nutc.edu.tw/curriculum/next_show_subject/check_show_select.asp -D "$SAV_COOKIE" -A "$BREW_UA" -e "$BRW_REF" -d "show_radio=2&show_select2="$(conv_2_big5 "$1") > /dev/null
	curl http://academic.nutc.edu.tw/curriculum/next_show_subject/show_subject_choose.asp -b "$SAV_COOKIE" -A "$BREW_UA" -e "$BRW_REF" | $ENC_CONV > show_subject_choose.asp 
	rm "$SAV_COOKIE"
...
function get_list(){
	## 取得一份完整的課程清單
	curl https://academic.nutc.edu.tw/curriculum/next_show_subject/show_subject_form.asp -A "$BRW_UA" > show_subject_form.asp 

	## 搜尋網頁第幾行有課程內容
	DATA_LINE=$(cat show_subject_form.asp | grep show_select2 -n | sed s/\:.*//g)
...
function list_URL(){
	...
	cat URL.txt | sed s/?flow_no/'http:\/\/academic.nutc.edu.tw\/curriculum\/next_show_subject\/rot_show_teach_flow.asp?flow_no'/g > FullURL.txt
}
...
function get_LessTime() {
	## 取得單一課程的上課時間
	curl "http://academic.nutc.edu.tw/curriculum/next_show_subject/rot_title_time.asp"$1 -D "$SAV_COOKIE" -A "$BREW_UA" -e "$BRW_REF" > /dev/null
	curl "http://academic.nutc.edu.tw/curriculum/next_show_subject/title_time.asp" -b "$SAV_COOKIE" -A "$BREW_UA" -e "$BRW_REF" | $ENC_CONV > time.html
	rm "$SAV_COOKIE"
...
do
	#echo "http://academic.nutc.edu.tw/curriculum/next_show_subject/rot_title_time.asp"$URL
	get_LessTime "$URL"
done
rm URL_TIME.txt
